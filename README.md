...
rm -rfv /tmp/aurora-rails-5-v3-web-server
sudo apt -y update 
sudo apt install -y git
git clone https://github.com/guanting112/aurora-rails-5-v3-web-server.git --branch master --single-branch /tmp/aurora-rails-5-v3-web-server

# 0-machine-setting.sh 是整套腳本的基本設定，可以讓你設定 git repo、ssh key、ssh port、部署帳號的名稱、密碼等等
# 您可以修改它 或 自行上傳一份自行製作的版本來複寫掉這個檔案
cat /tmp/aurora-rails-5-v3-web-server/0-machine-setting.sh

# or

vi /tmp/aurora-rails-5-v3-web-server/0-machine-setting.sh

# 當準備上述指令執行完成後，你需要執行以下片段，進行為安裝環境初始化
. /tmp/aurora-rails-5-v3-web-server/0-init-deploy.sh

# 部署帳號預設名稱為 apps，這組帳號將會成為您日後部署 rails 時專用的 user

# 完整的腳本資料會放在 aurora-rails-5-v3-web-server 目錄下
ls /home/apps/aurora-rails-5-v3-web-server
...
. ~/aurora-rails-5-v3-web-server/1-setup.sh
