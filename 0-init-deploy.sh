...
load_setting_file()
{
  . /tmp/aurora-rails-5-v3-web-server/0-machine-setting.sh
  rm /tmp/aurora-rails-5-v3-web-server/0-machine-setting.sh  
}
...
copy_setup_scripts()
{
  ...
  sudo rm -rfv /home/$DEPLOY_USER_NAME/aurora-rails-5-v3-web-server/0-init-deploy.sh > /dev/null
  sudo chown $DEPLOY_USER_NAME:$DEPLOY_USER_NAME /home/$DEPLOY_USER_NAME/aurora-rails-5-v3-web-server/*
  sudo chmod +x /home/$DEPLOY_USER_NAME/aurora-rails-5-v3-web-server/1-setup.sh
}
